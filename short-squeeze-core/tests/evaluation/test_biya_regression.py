...
def test_biya_momentum_is_evaluated_only_where_historical_inputs_support_it():
    for boundary in (EARLIEST, LATEST):
        ...
        assert rules["PERCENTAGE_CHANGE_MINIMUM"].outcome is RuleOutcome.UNKNOWN
        assert rules["RELATIVE_VOLUME_MINIMUM"].outcome is RuleOutcome.UNKNOWN
        assert rules["FLOAT_MAXIMUM"].outcome is RuleOutcome.UNKNOWN
...
def test_biya_short_pressure_missingness_never_becomes_failure_or_zero():
    for boundary in (EARLIEST, LATEST):
        ...
        assert pressure
        assert all(item.outcome in {RuleOutcome.UNKNOWN, RuleOutcome.INSUFFICIENT_DATA} for item in pressure)
        ...
        assert all(not item.input_observation_ids for item in pressure)
