...
#: The synthetic batch-05 fixture freezes one case per cohort symbol.
COHORT_SIZE = len(FROZEN_COHORT)
...
def test_generate_then_verify_round_trips(tmp_path):
    ...
    assert len(list((out / "requests").glob("*.json"))) == COHORT_SIZE
    assert len(list((out / "results").glob("*.json"))) == COHORT_SIZE
    assert len(list((out / "metrics").glob("*.json"))) == COHORT_SIZE
    assert len(list((out / "evidence-associations").glob("*.json"))) == COHORT_SIZE
    assert len(list((out / "leakage").glob("*.json"))) == COHORT_SIZE
