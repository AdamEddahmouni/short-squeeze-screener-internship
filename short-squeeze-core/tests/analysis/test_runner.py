...
def test_historical_case_boundary_analysis_preserves_dependence():
    ...
    assert result.sample_size_assessments[0].state is SampleSizeState.VERY_SMALL
...
def test_historical_unique_symbol_analysis_selects_earliest_without_outcome():
    ...
    assert result.unique_symbol_count == 1
    assert result.boundary_count == 2
    assert result.cohort_membership.included_case_ids == ("BIYA_EARLIEST_BOUNDARY",)
    duplicate = next(
...
def test_registry_standard_cohorts_preserve_registry_id_and_quality():
    ...
    assert partial_result.case_count == 6
