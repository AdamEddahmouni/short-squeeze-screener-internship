"""Phase 3C descriptive / data-quality analysis over the batch 02 registry.

Offline and deterministic: the analyzer is run over the committed batch-02
registry fixture and must yield registry-only, descriptive-only results with no
complete cases, no confusion matrix, and no predictive-validation claim. Because
no lawful outcome window could be captured, batch 02 is still entirely
registry-only, exactly like batch 01.
"""
...
BATCH_REGISTRY = (
    ROOT / "tests" / "fixtures" / "acquisition" / "batch02"
...
def test_batch02_descriptive_analysis_is_registry_only_and_deterministic(tmp_path, capsys):
...
def test_batch02_partial_blocked_cohort_runs(tmp_path, capsys):
