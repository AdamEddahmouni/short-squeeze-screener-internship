...
from core.corroboration_track_record import OUTCOME_FIELDS, OUTCOMES_PATH, score_band, summarize_outcomes
...
# - corroboration_history.csv (timestamp, ticker, corroboration_score) - every cycle,
#   core/corroboration_history.py
...
# (same "did it work" pattern as evaluate_squeeze_score_outcomes.py). This is the one advisor ask
# that had never been evidence-checked: cross-provider corroboration (IB agrees with Schwab) was
# built and labeled on every row, but nobody had verified whether a corroborated pick actually
# performs better than an uncorroborated one - this answers that directly, with recorded outcomes
# instead of trusting the feature on faith. Not wired into the main app loop; run
# manually/periodically as history accumulates.
BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
PRIME_LOG_PATH = os.path.join(BASE_DIR, "data", "prime_log.csv")
SCORE_HISTORY_PATH = os.path.join(BASE_DIR, "data", "corroboration_history.csv")

# Only score rows old enough that "what happened next" is a real, settled answer rather than
# same-day noise - matches evaluate_squeeze_score_outcomes.py's own bar.
...
def load_score_history(path=SCORE_HISTORY_PATH):
    ...
    it) rather than raising, matching core/corroboration_history.py's own read tolerance."""
    history = {}
...
def _print_band_summary():
    """Avg return and hit-rate by corroboration score across ALL outcomes ever recorded, not just
    this run's batch. Reuses core/corroboration_track_record.py's summarize_outcomes() so this
    printout and the web UI's Corroboration Track Record panel never drift apart. Also prints the
    SPY-relative alpha when available (2026-07-17) - see that module's docstring."""
    summary = summarize_outcomes()
    if not summary:
        return
    print("\nCorroboration score performance (all recorded outcomes):")
    for row in summary:
        line = (f"  score={row['score_band']}: n={row['n']:<4} "
...
def main():
    ...
    if not history:
        print(f"No corroboration history yet at {SCORE_HISTORY_PATH} - nothing to join against.")
        return
    ...
    if not new_outcomes:
        print("No new prime_log.csv rows old enough (with matching corroboration history) to score yet.")
        return
